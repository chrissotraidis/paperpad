...
Updated 2026-08-11 (Europe/Budapest). This file distinguishes reproduced acceptance from planned work.
...
| iPad Simulator | **Current development build verified** | Stateful audio PCM comparison, crash-log rotation, live Auto 7x/2240×1680 status, first-level diagnostics, modal touch restoration, borderless maximum-size 4:3 framing, private level-27 Toad Town loading/movement, and a direct drawable/frame-pacing leak regression passed; earlier evidence covers File 1A/Mario's House at 4x |
| Physical iPad | **Corrected iteration installed; release blocked on progression and audio** | The ROM/save-free 2026-08-11 arm64 replacement (executable SHA-256 `9f327e2190649848bf595a0e25797ddca6d950e348a2553fa0bb844dc5758ffa`) built, signed, installed in place, and launched on iPad Pro 12.9-inch (6th generation), iPadOS 26.5.2. The exact merged save hash remained `c7bd66d4e2e971796f636dab6abdedc39d06410a3c142a953168c402bf61e398`. Hands-on testing then encountered a Goomba Village gate-unlock freeze that left Mario input-locked and NPCs idle; audible synthesis crackle also remains release-blocking |
...
- A bounded source/output capture isolated one audio defect to the stateless per-block sample-rate converter. The clean 32 kHz source had `1.55e-6` of power above 14 kHz while the old 48 kHz output had `8.73e-5`; a continuous `SDL_AudioStream` reduced output to `1.36e-6`, with no conversion/queue error or queue depth above 100 ms. Later physical-iPad listening confirmed smaller audible crackle remains. The healthy queue and sub-full-scale PCM point to the HLE N64 synthesis path rather than an output gain or cache-size defect.
- The exact game microcode reports standard NAUDIO signature `0x0000127c`. A bounded attempt to use current standalone RSPRecomp output failed immediately because `n_aspMain` lacks the RSP boot-provided register and DMEM state. The experiment was removed and the stable HLE build restored; a complete boot+audio RSP backend remains the evidence-based implementation direction.
...
- SDL controller mappings cover hot-plug, left stick/D-pad, A/B/Start, L/R/Z, and right-stick C buttons. The installed iOS build hides gameplay touch controls while connected, keeps the utility menu available, and restores the overlay on disconnect according to the saved setting. Physical controller acceptance is open.
...
- The current source compiles and signs as arm64 iPhoneOS. Strict verification passed for executable SHA-256 `4705532b2b5ebca7752ed0fd78bcc6ae1ac7f19d29a3b7f7c831a857e34d6b33`, team `VKDH2T9UTF`, and the existing Xcode-managed wildcard profile. The ROM/save-free app installed in place without container reset, preserved the exact merged save, launched, and remained listed as PID 761 on the attached iPad. A fresh local clone replayed the complete maintained patch series cleanly.
...
1. Preserve a pre-scene save and current/previous diagnostics, then reproduce the Goomba Village gate-unlock sequence three times. The sequence must complete, Start must respond promptly, and movement/NPC interaction must restore before this P0 progression blocker can close.
2. On the next physical-iPad iteration, re-run title/file select/File 1A/Mario's House listening, name-entry cardinal feel, normal analog movement, native settings actions, Auto status, diagnostics, Original/Fill Screen, File 2, battle targeting, and background/terminate lifecycle. Confirm the image-filter control is gone and Smooth is restored. Audio crackle remains a release blocker even if every UI/control check passes.
3. Run a complete-game or chapter-spanning regression and a 60+ minute soak on both Simulator device classes, including hands-on extreme-stick and name-entry checks.
4. Build, install, and exercise the current source on a physical iPhone before making an iPhone device claim.
5. Use the installed private File 2 to document named middle/late checkpoints, battles, transitions, music, saving, lifecycle recovery, and a chapter-spanning/60-minute route without changing File 1 or redistributing the fixture.
6. Verify the implemented controller mapping, hot-plug/reconnect, touch-overlay handoff, and menu availability on physical hardware; separately decide whether to add the reference hold-to-latch Z gesture and per-control VoiceOver elements.
7. Clean up the non-blocking iOS launch warnings: unbalanced UIKit appearance transition and duplicate Simulator accessibility class.
8. Investigate the RT64 `RenderPool in Metal is not implemented currently` diagnostic and document whether the feature is unused or needs an implementation.
9. Complete binary packaging, required third-party notices, signing/notarization, privacy, accessibility, and rights review before any public binary.
