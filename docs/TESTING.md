...
| Gate | Evidence | Result |
|---|---|---|
| Audio output path | A fresh 82,919-byte on-device log during reported flutter showed steady 32→48 kHz conversion, roughly 16–64 ms queued output, zero conversion errors, zero queue errors, no queue interval above 100 ms, and sub-full-scale PCM | Confirms the audible defect is not SDL queue starvation, cache exhaustion, or output clipping; HLE synthesis/LLE replacement remains open |
