...
The iPhone/iPad overlay tracks independent fingers for simultaneous input. It exposes a fixed visible analog stick with a wider pickup region, a D-pad, A/B/Z, C-buttons, L/R, and Start. The stick clamps its drawn knob and N64 vector from one fixed origin, applies a quadratic inner precision response, and zeros the minor axis when one axis dominates by 1.45×; the outer rim still reaches full magnitude and near-equal axes remain diagonal. Settings persist touch visibility/opacity and edited normalized positions separately from the game save. Phone and tablet layouts use independent schema keys so a correction on one form factor does not reset the other; `iphone.v8` uses the physically revised compact spacing, while `ipad.v4` retains the accepted tablet arrangement. The normalized layouts and modal input lifecycle adapt HarkinianPad's accepted interaction pattern to PaperPad's direct N64 input bridge. Selecting a control preserves its grab offset. Directional buttons move individually by default; a persisted Link/Unlink choice can bind the selected D-pad or C cluster for movement while retaining internal geometry and safe bounds. Per-control size, opacity, and visibility remain independent. Menu/settings transitions clear held input and suppress the gameplay targets.

macOS and iOS own controllers directly through SDL2's `SDL_GameController` API; this is not SDL3, a direct Apple GameController implementation, or an engine-managed controller layer. The adapter uses SDL2's current-controller enumeration, device/handle instance IDs, and `SDL_GameControllerGetAttached` connection check. Reconciliation runs at startup, on add/remove/remap events, after foreground resume, and once per second while active. Valid handles keep their player slot, stale handles close and release held input, a sole returning controller takes player 1, and additional controllers use the next free slot without displacing player 1. Paper Mario consumes player 1, while the four-slot ownership model prevents another controller from replacing it. Instance-ID/player-slot transitions are logged for device diagnosis; the SDL controller subsystem is not restarted. The input loop reports the aggregate controller state to UIKit, so gameplay touch controls are cleared and hidden while a controller is connected, the utility menu remains available, and the overlay restores on disconnect according to the saved Touch Controls setting. HarkinianPad's hold-to-latch Z gesture and per-control VoiceOver elements for the custom-drawn gameplay overlay are not implemented.
